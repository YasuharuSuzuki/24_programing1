...
def try_all(exec_python_path):
    ...
    error_count = 0
    for index, in1, out1 in zip(df_answer.index, df_answer['in'], df_answer['out']):
        answer_input_string = '\n'.join(eval(in1))
        ...
        for out_str1 in answer_outputs:
            stdout_str1 = all_temp.pop(0)
            if out_str1 != stdout_str1:
                print(f"Error! temp={stdout_str1}, answer={out_str1}, index={index} input_str={answer_input_string}")
                print(type(out_str1), type(stdout_str1))
                error_count += 1
            else:
